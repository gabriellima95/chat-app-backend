...
func TestUserRepositoryGetByID(t *testing.T) {
	...
	t.Run("case=must-get-user-by-id", func(t *testing.T) {
		sqlite.DB.Exec("DELETE FROM users")
		sqlite.DB.Exec("DELETE FROM user_chats")
		sqlite.DB.Exec("DELETE FROM refactor_chats")
		username := "abcdef"
		user := &models.User{
			Username: username,
			Password: "abc",
		...
		savedUser, err := userRepository.GetByID(user.ID)
	...
	t.Run("case=must-get-user-by-id-with-populated-chats", func(t *testing.T) {
		sqlite.DB.Exec("DELETE FROM users")
		sqlite.DB.Exec("DELETE FROM user_chats")
		sqlite.DB.Exec("DELETE FROM refactor_chats")
		username := "abcdef"
		...
		if len(savedUser.Chats) != 2 {
			t.Errorf("Must fetch the user with populated chats")
		}
	})
}
